export const SAVE_SHIPPING_ADDRESS = 'SAVE_SHIPPING_ADDRESS'
export const SAVE_PAYMENT_METHOD = 'SAVE_PAYMENT_METHOD'

export const ORDER_CREATE_REQUEST = 'ORDER_CREATE_REQUEST'
export const ORDER_CREATE_RESPONSE = 'ORDER_CREATE_RESPONSE'
export const ORDER_CREATE_FAIL = 'ORDER_CREATE_FAIL'

export const ORDER_DATA_REQUEST = 'ORDER_DATA_REQUEST'
export const ORDER_DATA_RESPONSE = 'ORDER_DATA_RESPONSE'
export const ORDER_DATA_FAIL = 'ORDER_DATA_FAIL'
export const ORDER_DATA_RESET = 'ORDER_DATA_RESET'


export const ORDER_PAY_REQUEST = 'ORDER_PAY_REQUEST'
export const ORDER_PAY_RESPONSE = 'ORDER_PAY_RESPONSE'
export const ORDER_PAY_FAIL = 'ORDER_PAY_FAIL'
export const ORDER_PAY_RESET = 'ORDER_PAY_RESET'

export const ORDER_DELIVER_REQUEST = 'ORDER_DELIVER_REQUEST'
export const ORDER_DELIVER_RESPONSE = 'ORDER_DELIVER_RESPONSE'
export const ORDER_DELIVER_FAIL = 'ORDER_DELIVER_FAIL'
export const ORDER_DELIVER_RESET = 'ORDER_DELIVER_RESET'

export const ORDER_LIST_MY_REQUEST = 'ORDER_LIST_MY_REQUEST'
export const ORDER_LIST_MY_RESPONSE = 'ORDER_LIST_MY_RESPONSE'
export const ORDER_LIST_MY_FAIL = 'ORDER_LIST_MY_FAIL'
export const ORDER_LIST_MY_RESET = 'ORDER_LIST_MY_RESET'

export const ORDER_LIST_ADMIN_REQUEST = 'ORDER_LIST_ADMIN_REQUEST'
export const ORDER_LIST_ADMIN_RESPONSE = 'ORDER_LIST_ADMIN_RESPONSE'
export const ORDER_LIST_ADMIN_FAIL = 'ORDER_LIST_ADMIN_FAIL'
export const ORDER_LIST_ADMIN_RESET = 'ORDER_LIST_ADMIN_RESET'


